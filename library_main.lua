--[[
    AccioLib - v12 (Slider Update)
    - Added CreateSlider (Toggle + Slider Hybrid)
...
function AccioLib:CreateWindow(title)
    ...
    TitleLabel.BackgroundTransparency = 1; TitleLabel.Position = UDim2.new(0, 15, 0, 0); TitleLabel.Size = UDim2.new(1, -100, 1, 0)
    TitleLabel.Font = Enum.Font.GothamBold; TitleLabel.Text = title or "Accio Library"; TitleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
    TitleLabel.TextSize = 18; TitleLabel.RichText = true; TitleLabel.TextXAlignment = Enum.TextXAlignment.Left
    ...
    local function createTopBtn(text, color, pos)
        ...
        btn.Parent = Buttons; btn.BackgroundTransparency = 1; btn.Position = pos; btn.Size = UDim2.new(0, 30, 1, 0)
        btn.Font = Enum.Font.GothamBold; btn.Text = text; btn.TextColor3 = color; btn.TextSize = 24; btn.ZIndex = 11
        return btn
    end
    ...
    CloseBtn.MouseButton1Click:Connect(function() is_destroyed = true; ScreenGui:Destroy() end)
    ...
    local TabContainer = Instance.new("ScrollingFrame", Sidebar); TabContainer.BackgroundTransparency = 1; TabContainer.Size = UDim2.new(1, 0, 1, -10); TabContainer.Position = UDim2.new(0, 0, 0, 5); TabContainer.ScrollBarThickness = 0
    ...
    function Window:CreateTab(name)
        ...
        TabButton.Text = name; TabButton.TextColor3 = Color3.fromRGB(160, 160, 160); TabButton.Font = Enum.Font.GothamBold; TabButton.TextSize = 18; TabButton.RichText = true; Instance.new("UICorner", TabButton).CornerRadius = UDim.new(0, 6)
        ...
        function Tab:CreateButton(text, callback, customColor, order)
            ...
            local Button = Instance.new("TextButton", Page); Button.BackgroundColor3 = baseColor; Button.Size = UDim2.new(1, -10, 0, 35); Button.Font = Enum.Font.GothamBold; Button.Text = text; Button.TextColor3 = Color3.fromRGB(230, 230, 230); Button.TextSize = 16; Button.LayoutOrder = order or 0; Instance.new("UICorner", Button).CornerRadius = UDim.new(0, 6)
        ...
        function Tab:CreateToggle(text, default, callback, order)
            ...
            local ToggleFrame = Instance.new("Frame", Page); ToggleFrame.BackgroundColor3 = Color3.fromRGB(45, 45, 45); ToggleFrame.Size = UDim2.new(1, -10, 0, 35); ToggleFrame.LayoutOrder = order or 0; Instance.new("UICorner", ToggleFrame).CornerRadius = UDim.new(0, 6)
            local ToggleLabel = Instance.new("TextLabel", ToggleFrame); ToggleLabel.BackgroundTransparency = 1; ToggleLabel.Position = UDim2.new(0, 15, 0, 0); ToggleLabel.Size = UDim2.new(1, -60, 1, 0); ToggleLabel.Text = text; ToggleLabel.TextColor3 = Color3.fromRGB(230, 230, 230); ToggleLabel.TextSize = 15; ToggleLabel.Font = Enum.Font.GothamBold; ToggleLabel.TextXAlignment = Enum.TextXAlignment.Left
        ...
        -- CreateSlider(text, defaultState, callback, defaultVal, min, max, order)
        function Tab:CreateSlider(text, defaultState, callback, defaultVal, min, max, order)
            local Toggled = defaultState or false
            local CurrentValue = defaultVal or min
            
            local SliderFrame = Instance.new("Frame", Page)
            SliderFrame.BackgroundColor3 = Color3.fromRGB(45, 45, 45); SliderFrame.Size = UDim2.new(1, -10, 0, 65); SliderFrame.LayoutOrder = order or 0; Instance.new("UICorner", SliderFrame).CornerRadius = UDim.new(0, 6)
            
            -- Toggle Section
            local ToggleLabel = Instance.new("TextLabel", SliderFrame); ToggleLabel.BackgroundTransparency = 1; ToggleLabel.Position = UDim2.new(0, 15, 0, 5); ToggleLabel.Size = UDim2.new(1, -100, 0, 30); ToggleLabel.Text = text; ToggleLabel.TextColor3 = Color3.fromRGB(230, 230, 230); ToggleLabel.TextSize = 15; ToggleLabel.Font = Enum.Font.GothamBold; ToggleLabel.TextXAlignment = Enum.TextXAlignment.Left
            ...
            -- Slider Section
            local SliderBack = Instance.new("Frame", SliderFrame); SliderBack.BackgroundColor3 = Color3.fromRGB(60, 60, 60); SliderBack.Position = UDim2.new(0, 15, 0, 45); SliderBack.Size = UDim2.new(1, -80, 0, 6); Instance.new("UICorner", SliderBack).CornerRadius = UDim.new(1, 0)
            ...
            local ValueDisplay = Instance.new("TextLabel", SliderFrame); ValueDisplay.BackgroundTransparency = 1; ValueDisplay.Position = UDim2.new(1, -60, 0, 33); ValueDisplay.Size = UDim2.new(0, 50, 0, 30); ValueDisplay.Font = Enum.Font.GothamBold; ValueDisplay.Text = tostring(CurrentValue); ValueDisplay.TextColor3 = Color3.fromRGB(200, 200, 200); ValueDisplay.TextSize = 14
